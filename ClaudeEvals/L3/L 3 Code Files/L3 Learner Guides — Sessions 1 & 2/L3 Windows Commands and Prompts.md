...
## Windows (PowerShell) Edition — Sessions 1 & 2, All Prompts & Commands

This document consolidates every command, prompt, checklist, and reference table from the L3 Learner Guides (Sessions 1 & 2), rendered for **Windows (PowerShell)** only.
...
4. Open a NEW PowerShell or CMD window
```
```
claude --version
```
...
```
Expand-Archive L3_P1_claims_intelligence.zip -DestinationPath $env:USERPROFILE\Desktop\
cd p1_claims_intelligence
...
```
# Make it executable — Windows needs Git Bash to run .sh files directly.
# From a Git Bash terminal:
chmod +x scripts/run_eval_gate.sh
```
*Alternatively, invoke it without chmod from PowerShell via: bash scripts/run_eval_gate.sh*
...
```
*Run from Git Bash, or prefix with 'bash' from PowerShell.*
...
bash scripts/run_eval_gate.sh
```
*Via Git Bash, or prefix with 'bash' from PowerShell.*
```
...
```
*Plain 'curl' in PowerShell is aliased to Invoke-WebRequest — use curl.exe for real curl behavior.*

FastAPI docs should load. Then verify the health endpoint and application behavior:


```
curl.exe 'http://localhost:8000/claims/count?status=in_review'
```
```
...
```
# Make it executable — via Git Bash:
chmod +x scripts/code_health.sh
```
*Or run directly with: bash scripts/code_health.sh*
```
bash scripts/code_health.sh
```
